...
extension SigningView {
    private func _start() {
        ...
        // Capture the effective version AND effective ID before signing
        let effectiveVersion = _temporaryOptions.appVersion ?? app.version ?? ""
        let effectiveID = _temporaryOptions.appIdentifier ?? app.identifier ?? ""
        ...
        FR.signPackageFile(
            ...
            if let error {
                ...
                // Now we use the effectiveID we calculated above, instead of relying on the optional _temporaryOptions
                // And we check if the user has enabled the remove old version option
                if !effectiveID.isEmpty && _temporaryOptions.post_removeOldVersion {
                    _removeOlderVersions(identifier: effectiveID, currentVersion: effectiveVersion)
                }
    ...
    private func _removeOlderVersions(identifier: String, currentVersion: String) {
        let request = NSFetchRequest<Signed>(entityName: "Signed")
        request.predicate = NSPredicate(format: "identifier == %@", identifier)
        
        do {
            let apps = try viewContext.fetch(request)
            for app in apps {
                // If the version string doesn't match the one we just signed, delete it
                if app.version != currentVersion {
                    Storage.shared.deleteApp(for: app)
                }
            }
        } catch {
            print("Failed to remove older versions: \(error)")
        }
    }
}
